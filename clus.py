...
def cluster_assign(images_lists, dataset):
    ...
    for cluster, images in enumerate(images_lists):
     ...
     # Updated transform pipeline without ToTensor as images are already tensors
    print(image_indexes)
    normalize = transforms.Normalize(mean=[0.485], std=[0.229])  # MNIST is grayscale
...
def run_kmeans(x,x1,nmb_clusters, verbose=False):
    ...
    return [int(n[0]) for n in I],[int(n[0]) for n in B], losses[-1]
...
class Kmeans(object):
    ...
    def cluster(self, data,data1, verbose=False):
        ...
        # cluster the data
        I,B,loss = run_kmeans(data, data1,self.k, verbose)
        self.images_lists = [[] for i in range(self.k)]
        self.images_lists1=[[] for i in range(self.k)]
        for i in range(len(data)):
            self.images_lists[I[i]].append(i)
        for j in range(len(data1)):
            self.images_lists1[B[j]].append(j)
        if verbose:
